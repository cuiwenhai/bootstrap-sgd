...
result<-function(){
    ...
    x<-array(runif(100,0,5),c(100,1))#协变量
    thet<-c(1) #要估计的参数
...
btsgd<-function(x,y,error,maxiter,step=0.001)#B是bootsrap总次数,b是第几次bootstrap
  { 
      m<-nrow(x)
      ...
      k<-0  #第k个样本
      newerror<-1 
      result1<-matrix(rep(0,maxiter),maxiter,1)
    ...
    result2<-btsgd(x,y,error=1,maxiter=800,step=0.001)
